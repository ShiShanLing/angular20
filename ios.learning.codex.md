...
> Coordinator、Publisher、Subscriber、Subject、Cancellable
...
> 下列「迁移」指技术栈演进、工程重构与阶段衔接，不是指把本仓库迁到 iOS。按本路线学习时，提前知道这些分叉，可避免重复造轮子或过早全量重写。
...
| 从                                  | 到                      | 注意点                                                                                                         |
| ---------------------------------- | ---------------------- | ----------------------------------------------------------------------------------------------------------- |
| Storyboard / XIB                   | 纯代码 Auto Layout        | 阶段 2 就应练纯代码；老项目维护时常并存，不必一次删光 Storyboard                                                                     |
| UIKit 为主                           | SwiftUI 为主             | **渐进式**：`UIHostingController`（UIKit 包 SwiftUI）、`UIViewRepresentable`（SwiftUI 包 UIKit）；新功能可 SwiftUI，老模块别强行全改 |
| `NavigationView` 等旧 API            | `NavigationStack`      | 新工程直接用新 API；读老教程/源码时要会对照                                                                                    |
| `@ObservedObject` / `@StateObject` | `@Observable`（iOS 17+） | 数据流模型在变；与写新代码要两套都能讲清                                                                                      |
| 命令式布局                              | 声明式（SwiftUI）           | 思维迁移比语法更难：状态上提、单向数据流、少在 View 里写副作用                                                                          |


**原则（阶段 3 检查标准）**：能判断「何时 SwiftUI、何时 UIKit」，而不是二选一宗教战争。
...
| 从                         | 到                                               | 注意点                                                       |
| ------------------------- | ----------------------------------------------- | --------------------------------------------------------- |
| 回调 / delegate _completion | GCD / `OperationQueue`                          | 阶段 5 必学；遗留代码、系统 API 仍大量存在                                 |
| GCD                       | Swift Concurrency（`async/await`、`actor`、`Task`） | **新代码优先 async**；老模块可包装为 `async` 再逐步替换，不必一夜改完              |
| Combine                   | Swift Concurrency                               | 二者可并存：UI 绑定、事件流可用 Combine；网络与业务异步优先 `async/await`；别为迁移而迁移 |
| 主线程阻塞写法                   | `@MainActor` / 明确队列                             | 性能阶段（8）与并发阶段（5）要能对上：卡顿常源于迁移不彻底                            |


**原则**：先保证「可取消、线程安全、不堵主线程」，再谈框架统一。
...
| 从              | 到               | 注意点                                                          |
| -------------- | --------------- | ------------------------------------------------------------ |
| `UserDefaults` | Keychain / 加密文件 | 登录态、Token 等敏感数据**不要**长期放 UserDefaults（阶段 4、11）               |
| 无持久化           | SQLite / 文件     | 笔记、离线 App（阶段 4）先走通再上大框架                                      |
| Core Data      | SwiftData       | 新项目可优先 SwiftData；**已有 Core Data 库**要做模型与存储迁移方案，不能假设 API 一一对应 |
| 任意本地库          | 带版本的数据模型        | 阶段 4 明确「数据迁移」：改实体、改字段、轻量迁移 vs 重建，上线前要测旧用户升级路径                |


**原则**：持久化选型一次错，后期成本高于 UI 重写；作品集项目 3（记账）是练迁移的好时机。
...
| 从             | 到                          | 注意点                                             |
| ------------- | -------------------------- | ----------------------------------------------- |
| 单 Target 巨石工程 | 多 Target / 模块化             | 阶段 6；先划清模块边界与依赖方向，再拆 SPM                        |
| CocoaPods     | Swift Package Manager      | **新依赖优先 SPM**；老 Pod 可保留，迁移按模块逐步进行               |
| Carthage      | （了解即可）                     | 维护性弱，以认识为主，不必作为主路径                              |
| 手写 Xcode 工程   | Tuist / XcodeGen           | 大团队常用；个人学习可后置，与 CocoaPods/SPM 不要混用规则打架          |
| 单环境           | Dev / Staging / Production | 证书、Bundle ID、API 基址、Feature Flag 要一起设计（阶段 6、12） |


**原则**：依赖工具迁移是工程题，和业务架构迁移（MVVM 等）分开排期。
...
| 从                 | 到                         | 注意点                                 |
| ----------------- | ------------------------- | ----------------------------------- |
| MVC（系统默认）         | MVVM / MVP                | 阶段 6 实战：先把 Todo 改 MVVM，再谈 VIPER/TCA |
| 页面内逻辑             | Coordinator / Router      | 导航与深链从 VC 抽离，适合模块变多后                |
| 巨型 ViewController | 分层 + 依赖注入                 | 与「可测试」（阶段 7）绑定：能测的才算迁移成功            |
| 直接调 API           | 独立 Network / Repository 层 | 阶段 4 网络层封装是后续架构迁移的前置                |


**原则**：架构迁移用「一个 App 一条线」练手（阶段 6、14 项目 1→6），避免同时换 UI 框架 + 数据库 + 包管理。
...
| 从           | 到                          | 注意点                                                                |
| ----------- | -------------------------- | ------------------------------------------------------------------ |
| Objective-C | Swift                      | 阶段 9 底层 + 混编：新功能 Swift，老模块 `@objc` 桥接；KVO/KVC/Runtime 题多来自 ObjC 世界 |
| 仅会用 API     | 理解 Runtime / Mach-O / dyld | **读源码、排疑难**用（阶段 9、13），日常业务不必手写 Swizzling                           |
| Block 内存语义  | async / ARC 规则             | 重点高频；与「循环引用」（阶段 8）同一脉络                                             |


**原则**：底层是为了读懂系统与老代码，不是为了新项目全面 ObjC。
