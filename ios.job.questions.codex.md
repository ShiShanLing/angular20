...
- [Swift 基础](#swift-基础)
- [Swift 基础 / 值类型与引用类型](#swift-基础-值类型与引用类型)
- [Objective-C / Runtime](#objective-c-runtime)
- [UIKit / App 生命周期](#uikit-app-生命周期)
- [UIKit / 布局](#uikit-布局)
- [UIKit / 列表](#uikit-列表)
- [UIKit / 性能](#uikit-性能)
- [UIKit / 渲染](#uikit-渲染)
- [UIKit / 事件响应](#uikit-事件响应)
- [网络 / 安全](#网络-安全)
- [网络 / TCP](#网络-tcp)
- [网络 / HTTP](#网络-http)
- [网络 / 鉴权](#网络-鉴权)
- [网络 / URLSession](#网络-urlsession)
- [网络 / 架构](#网络-架构)
- [网络 / 缓存](#网络-缓存)
- [数据存储](#数据存储)

- [数据存储 / 安全](#数据存储-安全)
- [数据存储 / Core Data](#数据存储-core-data)
- [数据存储 / 迁移](#数据存储-迁移)
- [并发 / 线程](#并发-线程)
- [并发 / GCD](#并发-gcd)
- [并发 / OperationQueue](#并发-operationqueue)
- [并发 / RunLoop](#并发-runloop)
- [Swift Concurrency](#swift-concurrency)
- [内存管理](#内存管理)
- [内存管理 / 调试](#内存管理-调试)
- [架构 / MVC](#架构-mvc)
- [架构 / MVVM](#架构-mvvm)
- [架构 / Coordinator](#架构-coordinator)
- [架构 / 依赖注入](#架构-依赖注入)
- [架构 / 可测试性](#架构-可测试性)
- [架构 / 模块化](#架构-模块化)
- [工程化 / 配置](#工程化-配置)
...
- [性能 / 包体积](#性能-包体积)
- [性能 / 网络优化](#性能-网络优化)
- [性能 / Instruments](#性能-instruments)
- [Mach-O / dyld](#mach-o-dyld)
- [Mach-O / Runtime 初始化](#mach-o-runtime-初始化)
- [Mach-O / 包体积](#mach-o-包体积)
- [项目表达](#项目表达)
- [项目表达 / 架构](#项目表达-架构)
- [项目表达 / Bug 定位](#项目表达-bug-定位)
- [项目表达 / 性能](#项目表达-性能)
- [项目表达 / 网络层](#项目表达-网络层)
- [项目表达 / 缓存](#项目表达-缓存)
- [项目表达 / 重构](#项目表达-重构)
...
- [算法 / 缓存](#算法-缓存)
- [工程化 / 线上质量](#工程化-线上质量)
- [调试 / Xcode Debugger](#调试-xcode-debugger)
- [调试 / LLDB](#调试-lldb)
- [性能 / 卡顿与渲染](#性能-卡顿与渲染)
- [并发 / Timer](#并发-timer)
- [内存 / ARC](#内存-arc)
...
但不能绝对说 Struct 一定在栈上。实际放栈还是堆由编译器和使用场景决定，例如 Struct 被 class 持有时会存在堆对象内部；被逃逸闭包捕获、放进某些容器、发生装箱、使用协议 existential 或泛型时，也可能涉及堆存储。更稳的表达是：Struct 在常见局部使用场景下通常在栈上或内联存储，更容易减少堆分配和 ARC 成本；但 Struct 和 Class 的根本区别仍然是值语义和引用语义。
...
排查时可以用 Instruments 的 Core Animation，或者 Xcode 的 Debug View Hierarchy / Color Offscreen-Rendered 观察哪些图层触发了离屏渲染。总结：离屏渲染不是一定有问题，问题在于列表、动画这类高频场景大量触发，导致每帧渲染成本变高。
...
GCD 和 Swift Concurrency 的关系：GCD 更偏底层任务派发，关注把闭包提交到队列；async/await、Task、TaskGroup、actor、MainActor 是 Swift 的语言级并发模型，关注把异步流程写得更清楚，并集成取消、错误处理和结构化并发。现代业务异步流程通常优先 async/await 和 Task；老项目、底层队列控制、barrier、semaphore、一些系统 API 和相关场景里仍然需要理解 GCD。

总结：GCD 是基于队列的任务调度 API。串行/并发决定队列内部一次执行几个任务，sync/async 决定当前线程是否等待。常用能力有 main/global queue、asyncAfter、DispatchGroup、Semaphore、WorkItem、barrier。GCD 死锁的本质是同步等待形成等待环，最常见是主队列 sync 主队列、串行队列里 sync 自己、主线程 wait 等一个回主线程 signal 的任务。
...
总结：OperationQueue 是比 GCD 更面向任务对象的并发工具，适合管理复杂任务编排。它支持依赖、取消、优先级、最大并发数、暂停恢复和状态观察。常见坑是 cancel 不会强杀任务、suspend 不会暂停已运行任务、主线程 wait 会卡死、依赖环导致任务永远不执行、异步 Operation 要正确维护完成状态。
...
但也要注意：现在日常业务开发里，不建议为了普通异步任务去手写复杂 RunLoop。耗时任务可以优先用 async/await、Task、OperationQueue、GCD。RunLoop 更多用于理解系统事件机制、Timer、卡顿监控、常驻线程和底层原理。

可以这样回答：RunLoop 是线程的事件循环机制，让线程有事件时处理事件，没事件时休眠。主线程默认开启 RunLoop，所以 App 能持续响应点击、滚动、Timer 和 UI 刷新；子线程如果需要 Timer 或长期等待事件，需要手动启动 RunLoop。Timer 必须加入 RunLoop 的某个 Mode 才能触发，滚动时 Timer 暂停通常是因为 default mode 和 tracking mode 不匹配，可以加入 common modes。主线程如果被耗时任务阻塞，RunLoop 无法及时处理事件和刷新，就会造成卡顿。

总结：RunLoop 不是线程，而是线程的事件循环；主线程靠它活着并响应 UI 事件。Timer、Mode、滚动暂停、常驻线程、自动释放池和卡顿监控，都是 RunLoop 高频考点。
...
- 口述一句话：actor 方法里的 await 会让出 actor，恢复后状态可能已变化；await 后要重新校验状态，避免按旧状态写回。

**参考答案：**

**可先这样说（对齐题干口述）：**

actor 方法里的 `await` 会让出 actor，其他任务可能趁机修改 actor 内部状态；所以 `await` 之后不能盲目相信之前读到的状态，要重新校验后再写回。

---

**展开理解：**

actor 能保证同一时间只有一个任务执行它的隔离代码，但这个保证只覆盖“没有挂起的连续代码片段”。一旦 actor 方法执行到 `await`，当前方法会挂起并让出 actor，actor 可以先执行其他排队任务。等原方法从 `await` 恢复时，actor 内部状态可能已经被别的任务改过，这就是 actor reentrancy（可重入）带来的问题。

典型风险是“检查状态 -> await -> 按旧状态修改状态”。例如账户扣款时，先判断余额足够，然后 `await` 网络确认，最后再扣款。`await` 期间其他任务可能已经扣过款；如果恢复后仍然使用 await 前的判断结果，就可能写出错误状态。

应对方式：

1. 不要跨 `await` 依赖旧状态，`await` 之后重新读取并校验 actor 内部状态。
2. 把状态修改放在 `await` 前后清晰的同步片段里，避免“先判断、挂起、再按旧判断写回”。
3. `await` 前可以取不可变快照做外部请求，但真正写回 actor 状态时要再次校验。
4. 能放到 actor 外部的耗时异步工作，尽量先在外部完成，再回 actor 做短小的状态更新。
5. 复杂场景可以用状态机、任务去重、版本号或 id 校验，避免旧请求覆盖新状态。

收口一句：**actor 不是把整段 async 方法都锁住；`await` 是可重入边界，`await` 之后要假设状态可能已经变了。**
...
App 启动可以先按场景分为冷启动和热启动。冷启动是 App 进程原本不存在，用户点击图标后，系统从 0 开始创建进程、加载程序、初始化运行时，再进入 UIKit 的应用生命周期，最后由 App 自己完成首屏。热启动是 App 进程还在后台，只是重新回到前台，通常不需要重新走完整的进程加载流程，所以更快。

冷启动流程可以拆成三段。第一段是 main 前的系统加载流程：用户点击 App 图标，系统接收到启动请求；系统创建 App 进程，分配进程空间；加载 App 的 Mach-O 可执行文件；dyld 加载依赖的动态库，包括系统库、第三方动态库和 Framework；做 rebase、bind、符号绑定和重定位，让函数、类、全局变量能找到真实地址；初始化 ObjC/Swift Runtime，包括类注册、分类加载、方法表准备、协议和元数据处理；执行 +load、C/C++ 静态初始化和部分全局对象初始化；最后进入 main。

第二段是 main 后的 UIKit 系统流程：main 里调用 UIApplicationMain；系统创建 UIApplication；创建或关联 AppDelegate；建立主 RunLoop、事件分发和应用生命周期管理；处理启动参数；回调 application:didFinishLaunchingWithOptions:；iOS 13 以后还会创建 UISceneSession、连接 SceneDelegate，并回调 scene:willConnectToSession:options: 等 Scene 生命周期方法。

第三段才是 App 自己的启动代码：在 AppDelegate/SceneDelegate 回调内部初始化日志、网络、数据库、路由、登录状态、推送、埋点等业务依赖，创建 window、根控制器，加载首屏数据，布局 UI，完成首屏渲染。这一段属于应用层启动链路，不是系统流程，但会影响冷启动首屏耗时。
...
启动优化要先区分 main 前和 main 后。main 前主要是系统加载和运行时初始化，优化方向包括减少动态库数量、减少不必要的类和符号、少用 +load、避免启动期做太多 Runtime 注册或反射相关工作。

main 后主要是 App 自己的初始化和首屏链路，优化方向包括延迟初始化、懒加载、异步 IO、减少首屏网络依赖、减少复杂布局和大图片解码，把非首屏必须的任务后移。
...
iOS 启动优化本质是在优化用户点击 App 图标后，到首页可见、可操作之间的时间。这里可以先说：启动优化的核心是减少启动阶段主线程工作、减少动态库和类加载成本、延迟非必要初始化，让首屏尽快可见可操作。
...
总结：iOS 启动优化 = main 前减少加载成本，main 后减少启动路径上的业务耗时，让首屏尽快可见可操作；核心原则是启动阶段只做必须做的事，其他都延迟、异步、按需。
...
总结：列表滚动优化 = Cell 轻量 + 复用正确 + 图片异步可取消 + 高度稳定 + 局部刷新 + 布局渲染简单。真实项目中最容易遇到的是滑动掉帧、图片错乱、复用状态残留、动态高度跳动、内存上涨、频繁 reloadData 和数据源更新不一致。
...
可以这样说：图片性能主要关注下载、缓存、解码、缩放和复用取消。图片文件是压缩格式，真正显示前要解码成 Bitmap，大图如果在主线程解码会造成卡顿。实际项目里我一般用 SDWebImage，它会处理内存缓存、磁盘缓存、异步下载、后台解码、回主线程设置图片，并支持取消请求和缓存策略。优化上，服务端返回合适尺寸，客户端避免列表加载原图；Cell 复用时取消旧请求并设置 placeholder；URL 不变但内容变化时清缓存或让后端加版本号/hash；缓存要设置大小和过期时间；大图用 downsample，避免解码后内存暴涨。

总结：图片优化 = 合适尺寸 + 异步下载 + 后台解码 + 内存/磁盘缓存 + 复用取消 + 缓存策略。SDWebImage 封装了这些能力，但仍要注意大图、缓存 key、URL 版本、placeholder 和缓存上限。
...
Link Map 记录最终二进制里的目标文件、代码段、数据段和符号的大小分布。这里的代码段、数据段可以理解为 Mach-O 里不同用途的区域：代码段放函数和方法实现，数据段放全局变量、常量、字符串、静态数据等。通过 Link Map 可以找出大库、大函数、大静态数据、重复依赖和异常增长的符号。它常用于包体积分析，也能辅助判断哪些模块对二进制体积贡献最大。
...
示例：我最近负责的是一个电商类 iOS App 的商品详情和下单链路。这个项目的背景是老版本详情页加载慢、模块耦合比较重，运营经常要调整价格、优惠、推荐、评价等模块，但每次改动都容易影响主流程。我的职责主要是重构详情页模块、优化首屏加载速度，并把下单前的库存、优惠、地址校验流程梳理清楚。

技术上，项目主要使用 Swift、UIKit、MVVM，网络层基于 URLSession 封装，图片加载使用缓存和异步解码，页面模块按商品信息、价格优惠、评价、推荐、购买栏拆分。架构上我把业务逻辑放到 ViewModel 和 Service 里，ViewController 只负责组装 UI 和响应用户操作，避免页面越来越臃肿。

这个项目里比较有代表性的难点是详情页模块多、数据来源多，而且首屏不能等所有接口都返回。我的做法是把接口拆成首屏必要数据和非首屏数据：标题、价格、主图、库存这类优先加载；评价、推荐、活动说明延后加载。列表和模块刷新也尽量做局部更新，避免每次数据回来都全量刷新页面。同时我加了关键节点埋点，统计接口耗时、首屏展示时间和下单转化路径。

最终结果是详情页首屏展示时间下降了一段明显比例，用户能更快看到商品核心信息；代码结构也比原来清楚，后续新增优惠模块或推荐模块时，只需要新增对应模块和 ViewModel，不需要大改主页面。这个项目让我比较有收获的是：复杂页面不能只堆 UI，要先拆清楚业务边界、数据优先级和刷新策略。
...
示例：我负责过一个电商 App 的商品详情和下单链路，架构上主要按业务边界和通用能力拆。业务模块包括首页、商品详情、购物车、订单、支付、个人中心；基础模块包括网络层、图片加载、日志埋点、缓存、路由、基础 UI 组件和工具库。依赖方向是业务模块依赖基础模块，业务模块之间尽量不直接互相调用，比如商品详情不会直接依赖订单模块，而是通过路由或服务协议发起下单流程。

在商品详情内部，我按 MVVM 拆分。ViewController 只负责页面生命周期、组装 UI 和响应点击；ViewModel 负责把接口数据转换成页面模块数据；Service 负责商品详情、库存、优惠、推荐等接口；Cell 或 Section View 只关心自己的展示。数据流大致是 ViewController 触发加载，ViewModel 调用 Service 请求数据，数据回来后生成商品信息、价格优惠、评价、推荐、购买栏等 section model，页面再按 section 做局部刷新。

这样设计主要解决三个问题。第一是降低耦合，详情页不需要知道订单、支付内部怎么实现，只通过路由和协议跳转。第二是提升可维护性，运营新增一个优惠模块时，只需要新增对应的 section model 和 view，不用改整个页面。第三是方便测试，价格计算、库存状态、按钮置灰这些逻辑可以放在 ViewModel 里做单元测试。

不足是模块拆分后文件和协议会变多，新人理解成本会上升；另外如果抽象过度，小需求也可能要改好几个地方。后续我会控制模块粒度，只把稳定通用能力下沉，业务变化很快的部分先保持在业务模块内部，等边界稳定后再抽出来。
...
示例：我之前定位过一个商品详情页下单按钮偶发不可点击的问题。现象是部分用户进入详情页后，底部购买按钮一直是置灰状态，但刷新页面后又正常。线上日志显示不是大面积问题，主要集中在弱网和接口响应慢的场景。

我先从复现入手，用 Charles 做弱网模拟，把详情接口、库存接口、优惠接口分别延迟。后来发现当库存接口比详情接口晚很多返回时，按钮状态更容易异常。接着我在关键节点加日志，包括详情数据返回、库存数据返回、ViewModel 状态合并、按钮刷新时机和用户点击行为。日志显示库存接口已经返回有货，但按钮没有重新刷新成可点击。

继续排查后发现根因是 ViewModel 里有两个状态源：详情接口回来会生成默认购买栏状态，库存接口回来会更新库存状态。原来的代码只在详情接口回调里刷新底部按钮，库存接口回来后只更新了内部字段，没有触发对应 section 的局部刷新。弱网下两个接口顺序变化，就会导致 UI 停留在旧状态。

修复方式是把按钮状态收敛成一个统一的状态计算方法，任何影响购买按钮的字段变化后都调用同一个 updateBuyBarState，再通过 diff 或指定 section 做局部刷新。同时补了单元测试，覆盖详情先返回、库存先返回、库存失败、无货、有货等组合场景。上线前用 Charles 弱网和灰度日志验证，确认库存返回后按钮状态会正确刷新。

预防上，我把这类多接口合并状态的逻辑从 ViewController 移到 ViewModel，并规定 UI 不直接拼状态，而是只消费最终 ViewState。后续类似价格、优惠、库存这类会影响按钮状态的字段，都必须通过统一状态计算入口，避免再出现某个回调漏刷 UI 的问题。
...
性能案例必须有指标。说明优化前的耗时、FPS、内存或包体积，用什么工具定位到瓶颈，采用什么方案，优化后提升多少，以及有没有副作用。没有量化结果，对方很难相信优化价值。

示例：我做过一次商品详情页首屏和滑动性能优化。优化前的问题是详情页从点击商品到首屏可见大概在 1.8 秒左右，弱网下更明显；页面里评价、推荐、活动说明等模块比较多，滑动时偶尔掉帧，低端机上体验更差。

我先用埋点把链路拆开，记录点击商品、开始请求、详情接口返回、首屏数据组装完成、首屏 viewDidAppear 等节点；再用 Time Profiler 看主线程耗时，用 Core Animation 看滑动 FPS，用 Network 看接口耗时。定位下来主要有几个瓶颈：首屏等待了非必要接口，比如推荐和评价；ViewController 里一次性组装所有模块；图片有些尺寸过大，进入页面后主线程解码压力比较大；数据回来后多次 reloadData，导致 Cell 重复配置和布局。

优化方案分四步。第一，把接口分级，首屏只等标题、主图、价格、库存这些必要数据，评价、推荐、活动说明延后加载。第二，把模块组装从 ViewController 移到 ViewModel，并按 section 生成数据，接口回来后做局部刷新而不是全量 reloadData。第三，图片侧要求服务端返回合适尺寸，客户端使用缩略图、缓存和后台解码，Cell 复用时取消旧图片请求。第四，缓存部分稳定高度和布局结果，减少滑动时重复计算。

优化后，首屏可见时间从大约 1.8 秒降到 1.1 秒左右，低端机滑动时 FPS 更稳定，用户能更快看到商品主图和价格。副作用是页面会分阶段出现内容，比如推荐和评价稍后加载，所以我加了骨架态和占位，避免用户感觉页面突然跳动。这个案例的重点不是单一技巧，而是先量化链路，再区分首屏必要和非必要工作，把非首屏任务延后，并减少主线程重复布局和刷新。
...
总结：崩溃收集的核心是自动采集 crash、符号化堆栈、统计影响范围并辅助定位；常用工具有 Firebase Crashlytics、Bugly、Sentry、友盟+、阿里 ARMS、Datadog、Bugsnag 等。
...
Xcode 的断点不只是普通行断点，还可以按错误类型、异常类型、函数符号、运行时问题、约束冲突和测试失败来自动暂停程序。实际排查时，可以按“它们各自抓什么问题”来记。
...
总结：View Debugger 用来可视化查看当前 UI 层级、frame、约束和遮挡关系，最适合排查控件不显示、点击无效、布局错乱、Cell 高度异常和页面层级过深。
...
总结：LLDB 常用命令分四类：看变量用 `po/p`，改状态和执行代码用 `expr`，看调用栈用 `bt/thread backtrace all`，控制执行流程用 `continue/next/step/finish`。
...
总结：Timer 是 Foundation 的定时器，依赖 RunLoop 触发。scheduledTimer 默认加入当前 RunLoop 的 default mode，所以滚动列表时可能因为 RunLoop 切到 tracking mode 而暂停，可以加入 common modes。Timer 不保证绝对精准，主线程忙、mode 不匹配和系统调度都会造成延迟。使用 Timer 要注意循环引用，block 里 weak self，不需要时调用 invalidate 从 RunLoop 移除。
...
总结：iOS 常见循环引用集中在闭包、Timer、delegate、Notification、KVO、Combine、Task、CADisplayLink、对象互相 strong、Coordinator 和 Cell 回调。判断原则是看有没有强引用闭环；解决方式是 weak/unowned、invalidate、removeObserver、cancel、打断父子持有关系，并用 Memory Graph 和 deinit 验证。
