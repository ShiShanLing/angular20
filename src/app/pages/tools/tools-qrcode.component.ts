...
。
  ...
  async startCamera(): Promise<void> {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      ...
      this.cameraError.set(
        viaHttp
          ? '摄像头只能在 HTTPS 或 localhost 使用。请任选其一：1) 用 SSH 隧道后打开 http://localhost:18080/workshop/ ；2) Chrome 打开 chrome://flags/#unsafely-treat-insecure-origin-as-secure ，把 http://106.13.175.227 加进去并重启浏览器；3) 在百度云安全组放行 443 后用 HTTPS 访问'
